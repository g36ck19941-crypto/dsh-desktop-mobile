...
### 新增
- **个人面板升级为 ChatGPT 账户设置风格**：左侧导航（概览 / 消耗明细 / 版本更新 / 关于）+ 右侧居中限宽内容，设置项改为垂直列表（标题 + 描述 + 右侧控件），按钮统一右侧。
- **开放平台「刷新」按钮**：手动重新加载 platform.deepseek.com，更新余额/消费等数据。

### 修复
- **适配 DSH 新版会话格式（v3）**：消耗统计改为按版本号自动选会话日志文件（v3 优先），修复新会话不显示、老会话消耗不更新。
- **适配 DSH 新版 token 认证**：启动加 --no-open、自动解析 launch token 加载界面，DSH 面板改用 webview 承载。
- **DSH 版本锁定**：支持通过 dsh-pinned-version.txt 锁定/回退 DSH 版本。
